...
CONTAINS
    ...
    SUBROUTINE sip_hyperplane_init()
        ...
        DO i = 1, nmygrids
            ...
            CALL sip_hyperplane_init_grid(kk, jj, ii, mip(ip3), idx(ip3))
            ! Sorting the indices within each hyperplane for one grid
            CALL sip_hyperplane_sort_grid(kk, jj, ii, mip(ip3), idx(ip3))
            ! Checking the correctness of the arrays for one grid
            CALL sip_hyperplane_check_grid(kk, jj, ii, mip(ip3), idx(ip3))
        END DO
        ...
        ASSOCIATE ( &
            ...
            mip => mip_hp_f%arr, idx => idx_hp_f%arr)
